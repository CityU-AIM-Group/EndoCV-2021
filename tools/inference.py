...
def mymodel():
    ...
    checkpoint = torch.load('checkpoint/pranet_last.pth', map_location=torch.device('cpu'))
...
if __name__ == '__main__':
    ...
    for j in range(0, len(subDirs)):
        ...
        for imagePath in imgfiles[:]:
            ...
            #start.record()
            #
            _, _, _, outputs = model(images.unsqueeze(0))

            prediction = (outputs >= 0.5).float()
            prediction = tensor_to_PIL(prediction)

            prediction = prediction.resize((size[1], size[0]))
            prediction.save(saveDir +'/'+ filename +'_mask.jpg')
